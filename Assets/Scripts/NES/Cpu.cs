...
namespace Nes
{
    public class Cpu
    {
        ...
        public int Step()
        {
            ...
            if (_currentDmaCycle > 0)
            {
                ...
                if (_currentDmaCycle < TotalDmaCycles)
                {
                    ...
                    {
                        _emulator.Ppu.OamData = ReadMemoryByte((ushort)((_dmaPage << ByteLength) + _emulator.Ppu.OamAddr));
                    }
                    // Write
                    else
                    {
            ...
            {
                PushWordStack(_pc);
                PushByteStack((byte)(_p | Status.Bit5));
        ...
        {
            _p = (Status)PullByteStack() & ~(Status.Bit5 | Status.Bit4) | (_p & (Status.Bit4 | Status.Bit5));
        ...
        private byte ReadValue(AddressingMode mode, out bool crossedPage, bool oopsCycle = false)
        {
            ...
            switch (mode)
            {
                ...
                case AddressingMode.IndirectX:
                    var lo = ReadMemoryByte((ushort)(ReadOperandByte() + _x & LowByteMask));
                    var hi = ReadMemoryByte((ushort)(ReadOperandByte() + _x + 1 & LowByteMask));
        ...
        {
            WriteMemoryByte((ushort)(StackOffset | _s), value);
        ...
        {
            var address = (ushort)(StackOffset | _s - 1);
        ...
        private ushort PullWordStack()
        {
            ...
            return ReadMemoryWord((ushort)(StackOffset | _s - 1));
        }
    }
}
